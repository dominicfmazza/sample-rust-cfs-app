...
// const RUST_HK_TLM_MID: uint32 = 0x18FA;
const RUST_HK_TLM_MID: CFE_SB_MsgId_t = CFE_SB_MsgId_t { Value: 0x18FA };

const RUST_CMD_PIPE_DEPTH: uint16 = 10;

const RUST_CMD_PIPE_NAME: &str = "RustTlm";

const RUST_SEND_HK_MID: CFE_SB_MsgId_t = CFE_SB_MsgId_t { Value: 0x18F9 };
...
pub extern "C" fn Rust_AppInit() {
    ...
    let mut run_status: uint32 = CFE_ES_RunStatus_CFE_ES_RunStatus_APP_RUN;
    let mut app_id = 0;
    ...
    let mut rcv_packet: *mut CFE_SB_Msg = 0 as *mut CFE_SB_Msg;
    ...
    unsafe {
        ...
        CFE_ES_WriteToSysLog(
            CString::new("Rust App running as %d!\n").unwrap().as_ptr(),
            app_id,
        );
        ...
        if status != OS_SUCCESS as i32 {
            ...
            CFE_ES_WriteToSysLog(
                CString::new("RUST (%d) Registering with EVS failed!\n")
                    .unwrap()
                    .as_ptr(),
                app_id,
            );
        ...
        CFE_MSG_Init(
            mem::transmute::<&mut RustTlm, *mut CFE_MSG_Message>(mem::transmute(&mut tlm_packet)),
            RUST_HK_TLM_MID,
    ...
    while run_status == CFE_ES_RunStatus_CFE_ES_RunStatus_APP_RUN {
        unsafe {
